...
public class JacksonDBCollection<T, K> {
	...
	protected JacksonDBCollection(DBCollection dbCollection, JavaType type,
		...
		this.view = view;
		this.decoderFactory = new JacksonDecoderFactory<>(this, objectMapper,
				type);
	...
	public static <T> JacksonDBCollection<T, Object> wrap(
		...
		return new JacksonDBCollection<>(dbCollection,
	...
	public WriteResult<T, K> insert(T object) throws MongoException {
		...
		return new WriteResult<>(this, dbCollection.insert(dbObject),
				dbObject);
	}
	...
	public WriteResult<T, K> insert(T... objects) throws MongoException {
		...
		return new WriteResult<>(this, dbCollection.insert(dbObjects),
				dbObjects);
	}
	...
	public WriteResult<T, K> insert(WriteConcern concern, T... objects)
			throws MongoException {
		DBObject[] dbObjects = convertToDbObjects(objects);
		return new WriteResult<>(this, dbCollection.insert(concern,
				dbObjects), dbObjects);
	}
	 ...
	 * @param multi   if the update should be applied to all objects matching (db
	...
	public WriteResult<T, K> update(DBObject query, DBObject object,
			...
			throws MongoException {
		return new WriteResult<>(this, dbCollection.update(
	...
	public WriteResult<T, K> remove(DBObject query, WriteConcern concern)
			throws MongoException {
		return new WriteResult<>(this, dbCollection.remove(
	 ...
	 *
	 * @param keys   fields to use for index
	 * @param name   an identifier for the index. If null or empty, the default
	 *               name will be used.
	...
	public org.mongojack.DBCursor<T> find(DBObject query) throws MongoException {
		return new org.mongojack.DBCursor<>(this,
	...
	public <R, RK> List<R> fetch(
		...
		Map<JacksonCollectionKey, List<Object>> collectionsToIds = new HashMap<>();
	 ...
	 *
	 * @param object the object to save will add <code>_id</code> field to jo if
	 *               needed
	 * @return The result
	 */
	 ...
	 *
	 * @param newName    new collection name (not a full namespace)
	 * @param dropTarget if a collection with the new name exists, whether or not to
	 *                   drop it
	 * @return the new collection
	 * @throws MongoException If an error occurred
	 */
	public JacksonDBCollection<T, K> rename(String newName, boolean dropTarget)
			throws MongoException {
		return new JacksonDBCollection<>(dbCollection.rename(newName,
	 ...
	 * @param reduce   javascript reduce function
	 * @param initial  initial value for first match on a key
	 * @param finalize An optional function that can operate on the result(s) of the
	 *                 reduce function.
	 * @return The results
	 * @throws MongoException If an error occurred
	 * @see <a
	...
	public <S, L> MapReduceOutput<S, L> mapReduce(
		...
		return new MapReduceOutput<>(this, dbCollection.mapReduce(command
	...
	public <S> AggregationResult<S> aggregate(Aggregation<S> aggregation)
		...
		List<DBObject> serializedOps = new ArrayList<>();
		for (DBObject dbObject : aggregation.getAllOps()) {
			serializedOps.add(serializeFields(dbObject));
		}

		return new AggregationResult<>(this, dbCollection.aggregate(serializedOps), aggregation
				.getResultType());
	}

	public <S> AggregationResult<S> aggregate(Aggregation.Pipeline<?> pipeline, Class<S> resultType)
			throws MongoException {
		return new AggregationResult<>(this, dbCollection.aggregate(serializePipeline(pipeline)), resultType);
	}
	...
	public <T, K> JacksonDBCollection<T, K> getReferenceCollection(
		...
		JacksonDBCollection<T, K> collection = referencedCollectionCache
				.get(collectionKey);
		if (collection == null) {
			collection = new JacksonDBCollection<>(getDB().getCollection(
	...
	public List<T> convertFromDbObjects(DBObject... dbObjects) throws MongoException {
		final List<T> results = new ArrayList<>(dbObjects.length);
